...
- **模型审批（强管控）**：新发现的模型自动进入待审批区；**审批通过才会被发现与路由**，未审批/已拒绝模型自动写入 CPA 的 `excluded-models` / `oauth-excluded-models` 屏蔽清单，并在每次同步时幂等收敛、纠正漂移。
...
3. 保存后系统立即同步：新发现的模型进入 **模型 → 待审批**；审批「放行」后模型才会被发现，未放行模型由控制台自动写入 CPA 屏蔽清单。
